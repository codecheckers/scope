...
This code was straightforward to codecheck. There were two `.qmd` files containing R code to execute. One rendered with no issues whatsoever, and the other rendered successfully after I updated the data file name to that in the repository. Four figures were successfully reproduced. The only issue was that the figures are not saved from within the script, so their appearance (e.g., DPI, dimensions, aspect ratio) is different to that in the paper, but the visuals themselves show the same result.
...
- There were two `.qmd` files containing R code, so this is what the checked was conducted on.
- I began with `scope.qmd`. When prompted by RStudio, I installed the necessary packages (for me, this was only `pacman`).
- This rendered with no errors. The graphics outputted in the resulting `.html` were Figures 2, 3 and 4 in the paper. The visuals differed only in their aspect ratio, because the output is printed out rather than saved with specified dimensions and DPI. Other than that, all three figures visually matched those the paper.
- Rendering `discipline_figures.qmd` failed first time because the data specified at the beginning of the script did not exist. After changing the file name to the (only) worksheet on the repository `fulldata.xlsx` the document was built with no errors. The visual matches that of Figure 5 in the paper.
...
- I made a pull request for the data file name issue, since this is a clear improvement.
